...
from app.services.policy_service import POLICY_DEFAULTS, get_effective_policies, upsert_policies
...
def upsert_org_config(db: Session, organization_id: str, config: dict) -> dict:
    ...
    db.commit()
    db.refresh(row)
    # Bootstrap policy defaults on first config write.
    for key, value in POLICY_DEFAULTS.items():
        ...
        if existing is None:
            db.add(SystemConfig(organization_id=organization_id, config_key=key, config_value=value))
    db.commit()
...
def upsert_policy_config(db: Session, organization_id: str, policies: dict) -> dict:
    merged = upsert_policies(db, organization_id, policies)
    return {"version": "v1", "updated_at": datetime.now(timezone.utc).isoformat(), "policies": merged}


def get_policy_config(db: Session, organization_id: str) -> dict:
    merged = get_effective_policies(db, organization_id)
    return {"version": "v1", "updated_at": datetime.now(timezone.utc).isoformat(), "policies": merged}
