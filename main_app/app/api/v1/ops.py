...
async def upload_image(
    ...
    policies = policy_service.get_effective_policies(db, current_user["organization_id"])
    ...
    attendance_summary = attendance_service.mark_recognized_attendance(
        db,
    ...
    if attendance_summary.get("marked", 0) > 0:
        audit_service.write_audit_log(
            db,
            organization_id=current_user["organization_id"],
            user_id=current_user["user_id"],
