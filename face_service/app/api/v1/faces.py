...
from app.services.retention_service import purge_unknown_images
...
async def register_faces(
    ...
    policy_json: str | None = Form(None),
):
    policies = {}
    if policy_json:
        try:
...
def purge_retention(retention_days: int | None = None):
    data = purge_unknown_images(retention_days=retention_days)
    return {
        "success": True,
        "code": "RETENTION_PURGED",
